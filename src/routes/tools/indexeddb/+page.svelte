<script lang="ts">
	...
	async function loadDatabases() {
		try {
			if ('indexedDB' in window) {
				databases = JSON.parse(localStorage.getItem('indexeddb-databases') || '[]');
			}
		} catch (error) {
	...
	async function addData() {
		...
		try {
			...
			request.onsuccess = () => {
				const db = request.result;
				const transaction = db.transaction([selectedStore], 'readwrite');
				const store = transaction.objectStore(selectedStore);
			...
			alert('JSON 格式错误: ' + (error instanceof Error ? error.message : String(error)));
		}
	}
	...
	async function deleteRecord(key: any) {
		if (!selectedDb || !selectedStore || !confirm('确定要删除这条记录吗？')) return;
	...
	async function deleteDatabase(dbName: string) {
		if (!confirm(`确定要删除数据库 "${dbName}" 吗？这将删除所有数据！`)) return;
...
<svelte:head>
	...
	<meta name="description" content="功能完整的 IndexedDB 测试工具，支持数据库和表的创建、数据的增删改查、分页和搜索功能。" />
	<meta name="keywords" content="IndexedDB, 数据库测试, 前端存储, Web API, 数据管理" />
</svelte:head>
...
<div class="min-h-screen bg-gray-50 py-8">
	<div class="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8">
		...
		<div class="text-center mb-8">
			<h1 class="text-4xl font-bold text-gray-900 mb-4">IndexedDB 测试工具</h1>
		...
		<div class="bg-white rounded-lg shadow-md p-6 mb-8">
			<div class="flex flex-col sm:flex-row sm:items-center sm:justify-between mb-6">
				<h2 class="text-2xl font-semibold text-gray-900 mb-4 sm:mb-0">数据库管理</h2>
				<button
				...
				{#each databases as db}
					<div class="border border-gray-200 rounded-lg p-4 hover:shadow-md transition-shadow">
						<div class="flex justify-between items-start mb-2">
							<h3 class="font-semibold text-gray-900">{db.name}</h3>
							<button
						...
						<p class="text-sm text-gray-600 mb-2">版本: {db.version}</p>
						<p class="text-sm text-gray-600 mb-3">
							对象存储: {db.objectStores.length} 个
						</p>
						<button
							on:click={() => { selectedDb = db.name; handleDbChange(); }}
			...
			{#if selectedDb}
				<div class="bg-white rounded-lg shadow-md p-6 mb-8">
					...
					<!-- 对象存储选择 -->
					<div class="mb-6">
						<label for="store-select" class="block text-sm font-medium text-gray-700 mb-2">
							选择对象存储:
						</label>
						<select
							id="store-select"
				...
				<div class="bg-white rounded-lg shadow-md p-6 mb-8">
					...
					<div class="flex flex-col sm:flex-row gap-4 mb-6">
						...
						<div class="flex gap-2">
							<select
								bind:value={pageSize}
								on:change={() => { currentPage = 1; loadObjectStoreData(); }}
									...
									{#each currentData as record}
										<tr class="hover:bg-gray-50">
											{#each getObjectKeys(record) as key}
												<td class="px-6 py-4 whitespace-nowrap text-sm text-gray-900">
													{#if editingRecord === record}
														...
														/>
													{:else}
														<div class="max-w-xs truncate" title={formatValue(record[key])}>
											...
											<td class="px-6 py-4 whitespace-nowrap text-sm font-medium">
												{#if editingRecord === record}
													<button
														on:click={() => updateRecord(record)}
														class="text-green-600 hover:text-green-900 mr-3"
													>
														保存
													</button>
													<button
														on:click={() => editingRecord = null}
												...
												{:else}
													<button
														on:click={() => editingRecord = record}
														class="text-blue-600 hover:text-blue-900 mr-3"
													>
														编辑
													</button>
													<button
														on:click={() => deleteRecord(record[getObjectKeys(record)[0]])}
			...
			<!-- 使用说明 -->
			<div class="bg-white rounded-lg shadow-md p-6 mb-8">
				<h2 class="text-2xl font-semibold text-gray-900 mb-6">使用说明</h2>
				...
				<div class="grid grid-cols-1 lg:grid-cols-2 gap-8">
					...
					<div>
						<h3 class="text-lg font-semibold text-gray-800 mb-4">使用步骤</h3>
			...
			<div class="bg-white rounded-lg shadow-md p-6">
				<h2 class="text-2xl font-semibold text-gray-900 mb-6">技术原理与实现</h2>

				<div class="space-y-6">
					<div>
						<h3 class="text-lg font-semibold text-gray-800 mb-3">IndexedDB 简介</h3>
						<p class="text-gray-600 leading-relaxed">
							IndexedDB 是一个运行在浏览器中的非关系型数据库，它提供了比 localStorage 更强大的数据存储能力。
							支持事务、索引、大容量存储，并且是异步的，不会阻塞主线程。
						</p>
					</div>

					<div>
						<h3 class="text-lg font-semibold text-gray-800 mb-3">核心概念</h3>
						<div class="grid grid-cols-1 md:grid-cols-2 gap-4">
							<div class="bg-gray-50 p-4 rounded-lg">
								<h4 class="font-semibold text-gray-800 mb-2">数据库 (Database)</h4>
								<p class="text-sm text-gray-600">
									包含多个对象存储的容器，每个数据库有唯一的名称和版本号。
								</p>
							</div>
							<div class="bg-gray-50 p-4 rounded-lg">
								<h4 class="font-semibold text-gray-800 mb-2">对象存储 (Object Store)</h4>
								<p class="text-sm text-gray-600">
									类似于关系数据库中的表，用于存储 JavaScript 对象。
								</p>
							</div>
							<div class="bg-gray-50 p-4 rounded-lg">
								<h4 class="font-semibold text-gray-800 mb-2">事务 (Transaction)</h4>
								<p class="text-sm text-gray-600">
									确保数据操作的原子性，所有操作要么全部成功，要么全部失败。
								</p>
							</div>
							<div class="bg-gray-50 p-4 rounded-lg">
								<h4 class="font-semibold text-gray-800 mb-2">索引 (Index)</h4>
								<p class="text-sm text-gray-600">
					...
					<div>
						<h3 class="text-lg font-semibold text-gray-800 mb-3">实现技术</h3>
						<div class="bg-gray-50 p-4 rounded-lg">
							<ul class="space-y-2 text-gray-600">
								<li><strong>前端框架:</strong> Svelte 5 + TypeScript</li>
								<li><strong>样式框架:</strong> Tailwind CSS 4.0</li>
								<li><strong>数据存储:</strong> IndexedDB API + localStorage (数据库列表)</li>
								<li><strong>响应式设计:</strong> CSS Grid + Flexbox</li>
								<li><strong>状态管理:</strong> Svelte 响应式变量</li>
							</ul>
						</div>
					</div>

					<div>
						<h3 class="text-lg font-semibold text-gray-800 mb-3">浏览器兼容性</h3>
						<p class="text-gray-600 leading-relaxed">
							IndexedDB 在现代浏览器中有良好的支持，包括 Chrome 24+、Firefox 16+、Safari 10+、Edge 12+。
							本工具会自动检测浏览器支持情况。
						</p>
					</div>
				</div>
	...
	{#if showCreateDb}
		<div class="fixed inset-0 bg-black bg-opacity-50 flex items-center justify-center p-4 z-50">
			<div class="bg-white rounded-lg max-w-md w-full p-6">
				<h3 class="text-lg font-semibold text-gray-900 mb-4">创建新数据库</h3>

				<div class="space-y-4">
					<div>
						<label for="db-name" class="block text-sm font-medium text-gray-700 mb-1">
							数据库名称
						</label>
						<input
							id="db-name"
					...
					<div>
						<label for="db-version" class="block text-sm font-medium text-gray-700 mb-1">
							版本号
						</label>
						<input
							id="db-version"
	...
	{#if showCreateStore}
		<div class="fixed inset-0 bg-black bg-opacity-50 flex items-center justify-center p-4 z-50">
			<div class="bg-white rounded-lg max-w-md w-full p-6">
				<h3 class="text-lg font-semibold text-gray-900 mb-4">创建对象存储</h3>
				...
				<div class="space-y-4">
					...
					<div>
						<label for="store-keypath" class="block text-sm font-medium text-gray-700 mb-1">
							主键字段
						</label>
						<input
							id="store-keypath"
					...
					<div class="flex items-center">
						...
						/>
						<label for="store-autoincrement" class="ml-2 block text-sm text-gray-700">
							自动递增主键
						</label>
					</div>
				</div>
	...
	{#if showAddData}
		<div class="fixed inset-0 bg-black bg-opacity-50 flex items-center justify-center p-4 z-50">
			<div class="bg-white rounded-lg max-w-2xl w-full p-6">
				<h3 class="text-lg font-semibold text-gray-900 mb-4">添加新数据</h3>

				<div class="space-y-4">
					<div>
						<label for="data-json" class="block text-sm font-medium text-gray-700 mb-1">
							JSON 数据 (支持复杂对象结构)
						</label>
						<textarea
							id="data-json"
